...
def create_research_agent(codebase):
    ...
    # Create a prompt template with the system message and placeholders for chat history and user input
    ...
    if ANTHROPIC_API_KEY:
        llm = ChatAnthropic(temperature=0, model="claude-3-sonnet-20240229", max_tokens=4000)
    elif OPENAI_API_KEY:
        ...
        if OPENAI_API_ENDPOINT:
            ...
            llm = ChatOpenAI(temperature=0, model=OPENAI_MODEL, max_tokens=4000)
    else:
        raise ValueError("No API key found. Please set either ANTHROPIC_API_KEY or OPENAI_API_KEY in your environment variables.")
