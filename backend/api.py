...
def get_llm():
    ...
    # Check for OpenAI API key and custom endpoint
        ...
        if openai_api_base:
            print(f"Using custom OpenAI-compatible API at {openai_api_base}")
            return ChatOpenAI(
                temperature=0,
                model=openai_model,
    ...
    # Fallback to a default (this will likely fail without API keys)
    else:
        print("WARNING: No API keys found. Using default OpenAI configuration.")
